...
@onready var scrollingTextScene = preload("res://assets/addons/_3rdPackage/credits-scene/credits-scene/scrollingText.tscn")
@onready var creditsEnd = preload("res://assets/addons/_3rdPackage/credits-scene/custom/CreditsEnd.tscn")
...
@export var stringContent : String;
var _creditSceneEnd = null;
...
func _ready():
	...
	_creditSceneEnd = creditsEnd.instantiate()
	scrollingContainer.add_child(_creditSceneEnd)
...
func _process(delta):
	...
	if not done:
		...
		# if scrollingContainer.position.y+scrollingContainer.size.y - viewSize.y/2 > 0:
		# 	scrollingContainer.position.y -= speed*delta
		viewSize = get_viewport().size
		if (_creditSceneEnd.get_global_transform().origin.y  + offset_y >  _creditSceneEnd.get_viewport_rect().size.y / 2):
			scrollingContainer.position.y -= speed*delta
		else:
			end()
