...
var CustomAudioSystem := preload("res://addons/dialogic_additions/CustomAudioSystem/subsystem_custom_audio_system.gd").new():
	get: return get_subsystem("CustomAudioSystem")
...
func create_mask(arguments):
	var signal_data = SignalData.CreateSignal(
		SignalData.SignalEventType.CreateMask,
		arguments
	)
	var data = SignalData.CreateSignalFromString(signal_data);
	var dialogicNode = self.Styles.get_layout_node() ;
	effect_mask_obj_dict.node = DiologicUtils.create_mask_object(dialogicNode, data.content);
...
func SendSignalClickMask():
	print("Dialogic Game Handler Send signal")
	click_mask.emit()
