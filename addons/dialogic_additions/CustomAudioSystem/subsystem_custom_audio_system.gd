...
## Custom Subsystem that manages music and sounds.
...
func play_sound(path:String, volume:float = 0.0, audio_bus:String = "Master", loop :bool= false) -> Node:
	if base_sound_player != null and !path.is_empty():
		...
		return new_sound_node
	return null
		
func play_sound_callback(path:String, callback: Callable, volume:float = 0.0, audio_bus:String = "Master", loop :bool= false) -> Node:
	var finish_callback = func ():
		print("Callback after finish")
		callback.call()
	...
	if base_sound_player != null and !path.is_empty():
		...
		new_sound_node.play()
		new_sound_node.finished.connect(new_sound_node.queue_free)
		new_sound_node.finished.connect(finish_callback)
		return new_sound_node
	return null
